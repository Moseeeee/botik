...
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['правила', 'сап правила', '.правила', '!правила', '/rules@sapcmbot', '/rules', '/правила'])
...
# Определяем функцию get_report_content
...
# Определяем функцию обработчика сообщений
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['репорт', 'сап репорт', '.репорт', '!репорт', '/report@sapcmbot', '/report', '.жалоба'])
    ...
    try:
        ...
        if replied_message and report_text:
            ...
            for admin_chat_id in ADMIN_CHATS:
                await bot.send_message(admin_chat_id, report_message, parse_mode=types.ParseMode.MARKDOWN, reply_markup=inline_keyboard)
            ...
            await message.answer('Не удалось обработать вашу жалобу. Укажите причину и ответьте на сообщение, на которое отправляете жалобу.')
    except Exception as e:
        await message.answer('Произошла ошибка при обработке вашей жалобы. Пожалуйста, попробуйте еще раз позже.')  


#Команды
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['команды', 'сап команды', '.команды', '!команды', '/commands@sapcmbot', '/commands', '/команды'])
...
#Команды для админов:                
@dp.message_handler(lambda message: message.text.lower() == '+пин')
async def pin_message(message: Message):
    try:
        chat_admins = await bot.get_chat_administrators(message.chat.id)  # Получаем информацию об администраторах чата
        ...
        if is_admin and is_admin_can_pin:
            if message.reply_to_message:
                ...
                reply_message = await bot.send_message(message.chat.id, f"📌 [Сообщение](https://t.me/{(await bot.get_chat(chat_id=message.chat.id)).username}/{message.reply_to_message.message_id}) закреплено.", parse_mode="Markdown")
            else:
                await message.reply("Чтобы закрепить сообщение, ответьте на него командой '+пин'.", parse_mode=types.ParseMode.MARKDOWN)
        else:
            await message.reply("🔒 У вас недостаточно прав для закрепления сообщений", parse_mode=types.ParseMode.MARKDOWN)
    except Exception as e:
        await message.reply("🚫 У вас недостаточно прав для закрепления сообщений", parse_mode=types.ParseMode.MARKDOWN)

# Исправленный код для команды "-пин"
@dp.message_handler(lambda message: message.text.lower() == '-пин')
...
async def process_mute_duration(message: types.Message):
    if await user_can_restrict_members(message.from_user.id, message.chat.id):
        try:
            ...
            await bot.restrict_chat_member(message.chat.id, user.id, until_date=until_date, permissions=types.ChatPermissions())
            admin_link = f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
            user_link = f"[{user.first_name}](tg://user?id={user.id})"
            await message.answer(f"🔇 Пользователь {user_link} замучен.\n😺 Модератор: {admin_link}", parse_mode="Markdown")
        except Exception as e:
...
async def unmute(message: types.Message):
    if await user_can_restrict_members(message.from_user.id, message.chat.id):
        try:
            ...
            user = message.reply_to_message.from_user 
            await bot.restrict_chat_member(message.chat.id, user_id, permissions=types.ChatPermissions(can_send_messages=True, can_send_media_messages=True, can_send_other_messages=True, can_add_web_page_previews=True))
            admin_link = f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
...
async def ban_reply_user(message: types.Message):
    if await user_can_restrict_members(message.from_user.id, message.chat.id) and message.reply_to_message:
        try:
            ...
            admin_link = f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
            user_link = f"[{user.first_name}](tg://user?id={user_id})"
            await message.answer(f"🕒 Пользователь {user_link} забанен на {duration_val} {duration_unit}.\n😺 Модератор: {admin_link}", parse_mode="Markdown")
        except Exception as e:
            await message.answer(f"Произошла ошибка: {e}")
    else:
...
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['пиу', 'сап пиу', '.пиу', '!пиу', '/пиу', '?пиу'])
...
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['пинг', 'сап пинг', '.пинг', '!пинг', '/пинг', '?пинг'])
async def ping(message: types.Message):  
    start_time = time.time()
    sent = await message.answer('Подсчет...')
    end_time = time.time()
...
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['сап', 'сап сап', '.сап', '!сап', '/сап', '?сап'])
async def ping(message: types.Message): 
    start_time = time.time()
    sent = await message.answer('Подсчет...')
    end_time = time.time()
...
#Сетка чатов
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['сетка', 'сап сетка', '.сетка', '!сетка', '/grid@sapcmbot', '/grid'])
...
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['споти', 'сап споти', '.споти', '!споти'])
async def music_command(message: types.Message):
    ...
    current_time = time.time()
    command_timestamps[chat_id] = [timestamp for timestamp in command_timestamps[chat_id] if timestamp > current_time - 60]
    ...
    try:
        ...
        await bot.send_message(chat_id, f"Произошла ошибка при обработке запроса: {str(e)}. Пожалуйте, попробуйте снова.")
...
#Ссоры
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['ссоры', 'сап ссоры', '.ссоры', '!ссоры', '/ссоры'])
...
#Агрессия
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['агрессия', 'сап агрессия', '.агрессия', '!агрессия', '/агрессия'])
...
#Словарь
@dp.message_handler(lambda message: message.text.split()[0].strip().lower() in ['словарь', 'сап словарь', '.словарь', '!словарь'])
    ...
    try:
        ...
        if number > 0:
            ...
            if not error_message:
                ...
                await bot.send_sticker(message.chat.id, "CAACAgIAAxkBAAI2nGWFXKEM4_NOSx3PQJUcQuWqFU2RAALMFQACdWepSdykWqEAAVfrBDME")
                
        else:
            await bot.send_message(message.chat.id, "Пожалуйста, введите корректное число и убедитесь, что вы ответили на сообщение, с которого начать удаление.")
    ...
    if user_id in user_messages:
        ...
        if len(message_times) > message_limit and user_id not in flood_detected_users:
            if user_id not in last_alert_time or current_time - last_alert_time[user_id] >= alert_interval:
                await bot.send_message(admin_chat_id, f"В чате от пользователя {user_profile} замечена активность похожая на флуд. Надо бороться с флудом!", parse_mode="Markdown")
            ...
            await bot.send_message(admin_chat_id, f"Флуд от {user_profile} не прекращается. Пользователь получил мут на 30 минут.", parse_mode="Markdown")
            flood_detected_users.add(user_id)
